...
window.confirm = function(message) {
    ...
    return new Promise((resolve) => {
        ...
        modalWrapper.innerHTML = `
            <div class="absolute inset-0 bg-black/80 backdrop-blur-sm opacity-0 transition-opacity duration-300 modal-backdrop"></div>
...
window.openStatusSelectModal = function(options, currentStatus, title = 'Выберите новый статус') {
    ...
    return new Promise((resolve) => {
        ...
        modalWrapper.innerHTML = `
            ...
            <div class="relative w-full max-w-md bg-surface-container-highest border border-outline/10 rounded-[2.5rem] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)] flex flex-col transform scale-95 opacity-0 transition-all duration-300 min-h-0 overflow-hidden modal-content">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <nav id="globalHeader" class="fixed top-0 w-full z-[1000] bg-surface/90 backdrop-blur-md border-b border-outline-variant/20">
        <div class="flex justify-between items-center h-20 px-4 md:px-margin-edge w-full max-w-container-max mx-auto">
            <div class="flex items-center gap-2 min-[360px]:gap-3 md:gap-4 group">
                ...
                <a class="logo-link-capsule" href="/">
                    <div class="relative logo-img-container shrink-0">
                        <div class="absolute inset-0 bg-primary/30 blur-xl rounded-full opacity-40 logo-bg-glow transition-opacity duration-500"></div>
                        <img src="/images/logo_icon.png" alt="Железный Дровосек" class="logo-img object-cover relative z-10">
                    </div>
                    <div class="logo-text">
                        ЖЕЛЕЗНЫЙ <span>ДРОВОСЕК</span>
                    </div>
                </a>
            </div>
             <div class="hidden md:flex items-center gap-4 lg:gap-8 mx-auto whitespace-nowrap">
                <a class="nav-link font-label-caps text-[13px] text-on-surface-variant hover:text-primary transition-all duration-300 no-underline hidden lg:block" href="/">ГЛАВНАЯ</a>
    ...
    <div id="mobileMenuDrawerGlobal" class="fixed inset-0 z-[4000] pointer-events-none overflow-hidden md:hidden">
        ...
        <div id="mobileMenuPanelGlobal" class="absolute top-0 left-0 w-[85%] max-w-sm h-full bg-surface/90 backdrop-blur-[40px] border-r border-white/10 -translate-x-full transition-transform duration-500 ease-[cubic-bezier(0.33,1,0.68,1)] pointer-events-auto p-8 flex flex-col">
            <header class="flex justify-between items-center mb-10">
                <a href="/" class="logo-link-capsule">
                    <div class="relative logo-img-container shrink-0">
                        <div class="absolute inset-0 bg-primary/30 blur-xl rounded-full opacity-40 logo-bg-glow transition-opacity duration-500"></div>
                        <img src="/images/logo_icon.png" alt="Logo" class="logo-img object-cover relative z-10">
                    </div>
                    <div class="logo-text">
                        ЖЕЛЕЗНЫЙ <span>ДРОВОСЕК</span>
                    </div>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar pr-2">
                <div class="space-y-8 pb-10">
                    ...
                    <div class="pt-4 border-t border-white/5">
                        <a href="/catalog" onclick="toggleMobileCatalogGlobal()" class="flex items-center justify-center gap-2 w-full py-3.5 bg-primary text-on-primary text-center font-bold font-label-caps text-xs tracking-widest hover:bg-primary/90 transition-all uppercase rounded-xl no-underline">
    ...
    <div id="cartDrawerGlobal" class="fixed inset-0 z-[3000] pointer-events-none overflow-hidden">
        <div id="cartOverlayGlobal" class="absolute inset-0 bg-black/60 backdrop-blur-sm opacity-0 transition-opacity duration-150 pointer-events-none" onclick="toggleCartDrawerGlobal()"></div>
        <div id="cartPanelGlobal" class="absolute top-0 right-0 w-full max-w-md h-full liquid-glass border-l border-outline-variant/20 translate-x-full transition-transform duration-500 ease-in-out pointer-events-auto p-8 flex flex-col">
    ...
    <style>
        ...
        /* --- PREMIUM LOGO CAPSULE BUTTON STYLE --- */
        .logo-link-capsule {
            display: flex;
            align-items: center;
            gap: 12px;
            padding: 8px 20px 8px 8px; /* balanced padding */
            border-radius: 9999px;
            border: 1px solid rgba(202, 112, 147, 0.25);
            background: rgba(21, 19, 17, 0.3);
            box-shadow: inset 0 0 15px rgba(255, 255, 255, 0.01), 0 4px 15px rgba(0, 0, 0, 0.1);
        ...
        .logo-link-capsule:hover {
            border-color: #ca7093;
            box-shadow: inset 0 0 15px rgba(202, 112, 147, 0.05), 0 8px 25px rgba(202, 112, 147, 0.15);
            transform: translateY(-1px);
        }
        
        .logo-link-capsule .logo-img {
            width: 36px;
            height: 36px;
            border-radius: 50%;
            border: 1px solid rgba(202, 112, 147, 0.4);
            box-shadow: 0 0 10px rgba(202, 112, 147, 0.3);
            transition: transform 0.6s cubic-bezier(0.34, 1.56, 0.64, 1),
                        box-shadow 0.6s ease;
        }
        
        .logo-link-capsule:hover .logo-img {
            transform: scale(1.06) rotate(12deg);
            box-shadow: 0 0 20px rgba(202, 112, 147, 0.6);
        }
        
        .logo-link-capsule .logo-text {
        ...
        .logo-link-capsule .logo-text span {
            color: #ca7093;
            transition: text-shadow 0.3s ease;
        }
        
        .logo-link-capsule:hover .logo-text span {
            text-shadow: 0 0 12px rgba(202, 112, 147, 0.6);
        }
        
        /* Light Theme Adaptation */
        html.light .logo-link-capsule,
        html:not(.dark) .logo-link-capsule {
            background: rgba(255, 255, 255, 0.4) !important;
            border-color: rgba(202, 112, 147, 0.4) !important;
            box-shadow: inset 0 0 15px rgba(255, 255, 255, 0.4), 0 4px 15px rgba(0, 0, 0, 0.03) !important;
        }
        
        html.light .logo-link-capsule:hover,
        html:not(.dark) .logo-link-capsule:hover {
            border-color: #ca7093 !important;
            box-shadow: inset 0 0 15px rgba(252, 165, 185, 0.2), 0 8px 25px rgba(202, 112, 147, 0.12) !important;
        }
        
        html.light .logo-link-capsule .logo-text,
        html:not(.dark) .logo-link-capsule .logo-text {
            color: #1A1817 !important;
        }
        
        html.light .logo-link-capsule .logo-text span,
        html:not(.dark) .logo-link-capsule .logo-text span {
            color: #ca7093 !important;
        }
...
window.addToCartGlobal = function(id, qty = 1) {
    fetch(`/api/products/${id}`).then(res => res.json()).then(p => {
        ...
        toast.className = 'fixed bottom-8 right-8 bg-primary text-on-primary px-8 py-4 font-label-caps text-xs shadow-2xl z-[7000] rounded-full iron-toast-animate-in';
...
window.renderGlobalChatWelcomeScreen = function() {
    ...
    if (!container) return;
    container.innerHTML = `
        <div id="chatWelcomeState" class="flex flex-col items-center justify-center h-full py-8 px-4 animate-in fade-in duration-300">
...
window.openNewAppealTypePopup = function() {
    ...
    popup.innerHTML = `
        ...
        <div class="relative bg-surface-container-lowest border border-white/10 rounded-3xl shadow-2xl w-full max-w-sm opacity-0 transition-all duration-300 pointer-events-auto overflow-hidden" id="newAppealTypePanel">
        ...
        if (isMe) {
            ...
            return `
                <div class="flex flex-col items-start mb-4 animate-in fade-in duration-300">
                    <div class="flex items-end gap-2 max-w-[85%] md:max-w-[75%]">
                        <div class="relative px-5 pt-3 pb-5 pr-10 rounded-3xl text-sm bg-surface-container-high border border-outline-variant/30 text-on-surface rounded-bl-none shadow-md leading-relaxed break-words animate-in fade-in duration-300">
                            <span class="block">${m.text}</span>
...
(function() {
    ...
    // --- USER SESSION AND INACTIVITY MONITORING ---
    (function() {
        // 1. Fetch wrapper to capture new sliding session token
        ...
        window.fetch = async function(...args) {
            try {
                ...
                if (newToken) {
                    ...
                    if (window.setCookieGlobal) {
                        const rememberMe = localStorage.getItem('metal_remember') === 'true';
                        window.setCookieGlobal('metal_token', newToken, rememberMe ? 365 : null);
                    }
                    if (localStorage.getItem('metal_remember') === 'false') {
                        localStorage.setItem('metal_last_activity', Date.now().toString());
                    }
                }
                ...
                if (response.status === 403) {
                    ...
                    try {
                        ...
                        if (data && data.error && (
                            data.error.includes('Сессия истекла') || 
                            data.error.includes('сессия истекла')
                        )) {
                            // Only logout if NOT in the support chat drawer context (avoid flicker)
                            const isChatOpen = !!document.getElementById('globalChatDrawerModal');
                            if (!isChatOpen && window.handleLogoutGlobal) {
                                window.handleLogoutGlobal();
                                alert(data.error);
                            } else if (isChatOpen) {
                                // Close chat, show message, then logout
                                if (typeof window.closeGlobalChatDrawerGlobal === 'function') {
                                    window.closeGlobalChatDrawerGlobal();
                                }
                                setTimeout(() => {
                                    if (window.handleLogoutGlobal) {
                                        window.handleLogoutGlobal();
        ...
        // 2. Activity tracker to reset inactivity time
        const activityEvents = ['mousedown', 'keydown', 'scroll', 'touchstart'];
        function resetInactivityTimer() {
            const token = localStorage.getItem('metal_token');
            const remember = localStorage.getItem('metal_remember');
        ...
        // 3. Periodic session expiration checker
        setInterval(() => {
            const token = localStorage.getItem('metal_token');
            const remember = localStorage.getItem('metal_remember');
            
            if (token && remember === 'false') {
                const sessionStart = parseInt(localStorage.getItem('metal_session_start')) || Date.now();
                const lastActivity = parseInt(localStorage.getItem('metal_last_activity')) || Date.now();
                const now = Date.now();

                // 24 hour absolute limit
                if (now - sessionStart > 86400000) {
                    if (window.handleLogoutGlobal) {
                        window.handleLogoutGlobal();
                        alert('Время вашей сессии истекло (24 часа). Пожалуйста, войдите снова.');
                    }
                }
                // 30 minutes inactivity limit
                else if (now - lastActivity > 1800000) {
                    if (window.handleLogoutGlobal) {
                        window.handleLogoutGlobal();
                        alert('Вы были автоматически вылогинены из-за бездействия (30 минут).');
                    }
                }
            }
...
window.openProductCardModal = async function(productId) {
    ...
    try {
        ...
        const priceTon = Number(p.price_ton || p.priceTonNum || 0);
        const priceUnit = Number(p.price_unit || p.priceUnit || 0);
        const unitLabel = p.unit_label || p.unitLabel || 'т';
        ...
        modalContent.innerHTML = `
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start mt-4 max-h-[75vh] overflow-y-auto custom-scrollbar pr-2">
                ...
                <div class="space-y-6">
                    ...
                    <!-- Prices -->
                    <div class="p-4 bg-surface-container-lowest border border-outline/10 rounded-2xl space-y-2.5 shadow-inner">
                        <div class="flex items-center justify-between">
                            <span class="text-[10px] uppercase font-bold tracking-wider text-on-surface-variant/70 font-label-caps">Цена за тонну:</span>
                            <span class="text-base font-bold text-primary font-display-xl">${priceTon > 0 ? priceTon.toLocaleString() + ' ₽' : 'По запросу'}</span>
                        </div>
                        ${priceUnit > 0 ? `
                        <div class="flex items-center justify-between border-t border-outline/5 pt-2">
                            <span class="text-[10px] uppercase font-bold tracking-wider text-on-surface-variant/70 font-label-caps">Цена за ед. (${unitLabel}):</span>
                            <span class="text-sm font-bold text-on-surface font-display-xl">${priceUnit.toLocaleString()} ₽</span>
                        </div>` : ''}
                    </div>
                    ...
                    ${specs.length > 0 ? `
                    <div class="space-y-2">
                        <span class="text-[10px] uppercase font-bold tracking-wider text-on-surface-variant/60 font-label-caps block">Характеристики</span>
                        <div class="border-t border-outline-variant/10 text-xs">
                            ${specs.map(([label, value]) => `
                                <div class="flex justify-between py-2.5 border-b border-outline-variant/5">
                    ...
                    <!-- Description -->
                    <div class="space-y-2">
                        <span class="text-[10px] uppercase font-bold tracking-wider text-on-surface-variant/60 font-label-caps block">Описание</span>
                        <p class="text-xs text-on-surface-variant leading-relaxed opacity-85 font-body-md">${description}</p>
                    </div>

                    <!-- Actions -->
                    <div class="pt-4 border-t border-outline-variant/10">
                        <a href="/product?id=${productId}" target="_blank" class="w-full inline-flex items-center justify-center gap-2 px-5 py-3.5 bg-primary text-on-primary font-bold uppercase tracking-widest text-[10px] hover:brightness-110 active:scale-[0.98] transition-all rounded-xl no-underline text-center shadow-lg shadow-primary/10 font-label-caps" onclick="window.closeProductCardModal ? window.closeProductCardModal() : null">
                            <span class="material-symbols-outlined text-[16px]">open_in_new</span> Перейти к товару
                        </a>
                    </div>
                </div>
            </div>
        ...
        modalContent.innerHTML = `
            ...
            <div class="flex flex-col items-center justify-center py-12 text-red-400 gap-4 text-center">
                <span class="material-symbols-outlined text-4xl animate-bounce">error</span>
                <div class="text-sm font-medium">Не удалось загрузить данные о товаре.</div>
                <div class="text-xs opacity-60">${err.message}</div>
            </div>
        `;
        modalContent.querySelector('#close-product-card-modal').onclick = close;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (!localStorage.getItem('cookie_consent')) {
        const bannerHtml = `
        <div id="cookieBannerGlobal" class="fixed bottom-4 left-4 right-4 md:left-8 md:right-auto md:w-96 bg-surface-container-high/90 backdrop-blur-xl border border-outline-variant/30 p-6 rounded-2xl shadow-2xl z-[9999] transform translate-y-full transition-transform duration-500 ease-out">
            <h3 class="font-display-xl text-lg uppercase mb-2">Мы используем Cookie 🍪</h3>
            <p class="text-sm text-on-surface-variant mb-4 leading-relaxed">
                Этот сайт использует файлы cookie для улучшения пользовательского опыта. Продолжая работу с сайтом, вы соглашаетесь с нашей <a href="/privacy-policy.html" class="text-primary underline">Политикой конфиденциальности</a>.
            </p>
            <div class="flex gap-3">
                <button onclick="acceptCookiesGlobal()" class="flex-1 py-3 bg-primary text-on-primary font-label-caps text-xs tracking-widest hover:bg-primary/90 transition-all rounded-xl uppercase">СОГЛАСЕН</button>
            </div>
        </div>
        `;
