...
function updateBulkLeadsToolbar(state) {
        ...
        <div class="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-[#1d1b19]/90 backdrop-blur-md border border-[#ffb0cc]/20 px-6 py-4 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center gap-6 animate-in slide-in-from-bottom-8 duration-500">
            ...
            <div class="flex items-center gap-4">
                <div class="relative flex items-center gap-2">
                    <button type="button" id="bulk-lead-status-btn" class="flex items-center gap-2 bg-[#151311] border border-[#ffb0cc]/20 hover:border-[#ffb0cc]/40 rounded-xl px-4 py-2 text-xs font-bold uppercase tracking-widest text-[#ffb0cc] outline-none cursor-pointer hover:bg-white/5 transition-all shadow-md">
                ...
                <button type="button" id="bulk-lead-delete-btn" class="flex items-center gap-2 px-4 py-2 hover:bg-red-500/10 border border-red-500/20 rounded-xl transition-all text-xs font-bold uppercase tracking-widest text-red-400">
    ...
    closeBtn.onclick = () => {
        ...
        document.querySelectorAll('.lead-select-cell').forEach(cell => {
            ...
            if (block) block.className = "lead-select-block w-5 h-5 rounded border flex items-center justify-center transition-all cursor-pointer mx-auto border-white/20 bg-black/40 text-transparent";
        });
        const selectAll = document.getElementById('leads-select-all');
        if (selectAll) selectAll.className = "w-5 h-5 rounded border flex items-center justify-center transition-all cursor-pointer mx-auto border-white/20 bg-black/40 text-transparent";
    };
}
...
function renderLeadsTable(container, leads, state, typeMap, sortOptions = {}) {
    ...
    container.innerHTML = `
                ...
                <thead>
                    <tr class="border-b border-[#534347]/20 text-xs uppercase tracking-widest text-[#ffb0cc] font-['Space Grotesk']">
                        <th class="py-4 px-4 w-12 text-center">
                            <div id="leads-select-all" class="w-5 h-5 rounded border flex items-center justify-center transition-all cursor-pointer mx-auto ${allChecked ? 'bg-[#ffb0cc]/20 border-[#ffb0cc]/50 text-[#ffb0cc]' : 'border-white/20 bg-black/40 text-transparent hover:border-white/40'}">
                ...
                <tbody class="divide-y divide-[#534347]/10">
                    ${leads.map(lead => {
                        ...
                        <tr class="group hover:bg-white/[0.04] transition-colors cursor-pointer lead-row ${rowClass}" data-id="${lead.id}">
                            <td class="py-4 px-4 w-12 text-center lead-select-cell" data-id="${lead.id}">
                                <div class="lead-select-block w-5 h-5 rounded border flex items-center justify-center transition-all cursor-pointer mx-auto ${isSelected ? 'bg-[#ffb0cc]/20 border-[#ffb0cc]/50 text-[#ffb0cc]' : 'border-white/20 bg-black/40 text-transparent group-hover:border-white/40'}">
                            ...
                            <td class="py-5 px-6">
                                <div class="flex items-center gap-2">
                                    ${(() => {
                                        const statusObj = window.LEAD_STATUS_OPTIONS.find(o => o.value === status) || { label: status, color: '#3b82f6', icon: 'donut_large' };
                                        return `<button type="button" data-id="${lead.id}" data-status="${status}" class="lead-status-modal-btn flex items-center justify-between gap-2 bg-[#151311] border border-white/10 hover:border-white/20 rounded-xl px-4 py-2.5 text-xs font-bold uppercase tracking-widest transition-all shadow-lg group/btn w-[185px] shrink-0" style="border-left: 4px solid ${statusObj.color};">
                                            <span class="flex items-center gap-2 truncate" style="color: ${statusObj.color};">
                                                <span class="material-symbols-outlined text-base shrink-0">${statusObj.icon}</span>
                                                <span class="truncate">${statusObj.label}</span>
                                            </span>
                                            <span class="material-symbols-outlined text-sm text-[#d7c1c7] opacity-40 group-hover/btn:opacity-10 transition-opacity shrink-0 ml-1">expand_more</span>
                                        </button>`;
                                    })()}
                                    <button type="button" data-id="${lead.id}" class="delete-lead-btn p-2 hover:text-red-400 transition-colors" title="Удалить лид">
                                        <span class="material-symbols-outlined text-base">delete</span>
                                    </button>
                                </div>
                            </td>
        ...
        <div class="grid grid-cols-1 gap-4 md:hidden p-4">
            ${leads.map(lead => {
                ...
                return `
                    <div class="lead-row glass rounded-2xl p-5 border border-white/5 bg-[#151311] space-y-4 relative group cursor-pointer" data-id="${lead.id}" style="border-left: 4px solid ${statusObj.color};">
                        <div class="flex items-center justify-between gap-3 border-b border-white/5 pb-3">
                            <div class="flex items-center gap-3">
                                <div class="lead-select-cell" data-id="${lead.id}">
                                    <div class="lead-select-block w-5 h-5 rounded border flex items-center justify-center transition-all cursor-pointer mx-auto ${isSelected ? 'bg-[#ffb0cc]/20 border-[#ffb0cc]/50 text-[#ffb0cc]' : 'border-white/20 bg-black/40 text-transparent group-hover:border-white/40'}">
                                        <span class="material-symbols-outlined text-[14px] font-bold">check</span>
                                    </div>
                                </div>
                                <div>
                                    <div class="font-bold text-base uppercase text-[#e7e2dd] tracking-tight">${lead.name || 'Аноним'}</div>
                                    <div class="text-[10px] text-[#d7c1c7] opacity-60 font-mono mt-0.5">${new Date(lead.created_at).toLocaleString('ru-RU')}</div>
                                </div>
                            </div>
                            <span class="px-2.5 py-1 rounded-xl bg-[#ffb0cc]/10 border border-[#ffb0cc]/20 text-[10px] uppercase font-bold text-[#ffb0cc] tracking-widest font-['Space Grotesk'] inline-block shrink-0">
                                ${typeMap[lead.type] || lead.type || 'Заявка'}
                            </span>
                        </div>

                        <div class="space-y-2 text-xs">
                            <div class="text-[#ffb0cc] font-mono font-medium flex items-center gap-1.5 select-all">
                                <span class="material-symbols-outlined text-sm">call</span>
                                ${lead.phone || 'Телефон не указан'}
                            </div>
                            <div class="text-[#d7c1c7] font-mono flex items-center gap-1.5 select-all">
                                <span class="material-symbols-outlined text-sm">mail</span>
                                ${lead.email || 'Email не указан'}
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 pt-1 border-t border-white/5">
                            ${lead.project_type ? `<div class="text-[10px] text-[#ffb0cc] font-['Space Grotesk'] font-bold uppercase inline-flex items-center gap-1 bg-[#ffb0cc]/10 border border-[#ffb0cc]/20 px-2.5 py-1 rounded-lg"><span class="material-symbols-outlined text-xs">precision_manufacturing</span>Проект: ${lead.project_type}</div>` : ''}
                            <div class="text-xs text-[#e7e2dd] leading-relaxed italic line-clamp-3">
                                "${lead.message || 'Без комментария'}"
                            </div>
                        </div>

                        <div class="flex items-center justify-between gap-2 pt-2 border-t border-white/5">
                            <button type="button" data-id="${lead.id}" data-status="${status}" class="lead-status-modal-btn flex items-center justify-between gap-2 bg-[#151311] border border-white/10 hover:border-white/20 rounded-xl px-4 py-2.5 text-xs font-bold uppercase tracking-widest transition-all shadow-lg group/btn flex-1" style="border-left: 4px solid ${statusObj.color}; min-h-[44px]">
                                <span class="flex items-center gap-2 truncate" style="color: ${statusObj.color};">
                                    <span class="material-symbols-outlined text-base shrink-0">${statusObj.icon}</span>
                                    <span class="truncate">${statusObj.label}</span>
                                </span>
                                <span class="material-symbols-outlined text-sm text-[#d7c1c7] opacity-40 group-hover/btn:opacity-100 transition-opacity shrink-0 ml-1">expand_more</span>
                            </button>
                            <button type="button" data-id="${lead.id}" class="delete-lead-btn p-2.5 bg-white/5 hover:bg-red-500/10 hover:text-red-400 border border-white/10 rounded-xl transition-all flex items-center justify-center min-w-[44px] min-h-[44px]" title="Удалить лид">
                                <span class="material-symbols-outlined text-base">delete</span>
                            </button>
                        </div>
                    </div>
    ...
    container.querySelectorAll('.lead-select-cell').forEach(cell => {
        cell.addEventListener('click', (e) => {
            ...
            container.querySelectorAll(`.lead-select-cell[data-id="${id}"]`).forEach(c => {
                const block = c.querySelector('.lead-select-block');
                if (block) {
                    if (isChecked) {
                        block.className = "lead-select-block w-5 h-5 rounded border flex items-center justify-center transition-all cursor-pointer mx-auto bg-[#ffb0cc]/20 border-[#ffb0cc]/50 text-[#ffb0cc]";
                    } else {
                        block.className = "lead-select-block w-5 h-5 rounded border flex items-center justify-center transition-all cursor-pointer mx-auto border-white/20 bg-black/40 text-transparent group-hover:border-white/40";
                    }
                }
            });
    ...
    if (selectAll) {
        selectAll.onclick = e => {
            e.stopPropagation();
            const currentlyAllChecked = leads.length > 0 && leads.every(l => selectedLeadIds.has(l.id));
    ...
    container.querySelectorAll('.lead-status-modal-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            const newStatus = await window.openStatusSelectModal(window.LEAD_STATUS_OPTIONS, currentStatus, `Изменение статуса лида #${id}`);
            ...
            try {
                ...
                window.showToast(`Статус лида #${id} изменен на "${newStatus}"`, 'success', 'Обновление статуса');
                state.fetchLeads(); // refresh view
            } catch (err) {
    ...
    container.querySelectorAll('.delete-lead-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            e.stopPropagation();
            const id = btn.dataset.id;
            if (await confirm('Вы уверены, что хотите удалить этот лид?')) {
                try {
                    await state.authenticatedFetch(`/api/admin/leads/${id}`, {
                        method: 'DELETE'
                    });
                    // Refresh leads list
                    await state.fetchLeads();
                    window.showToast(`Лид #${id} успешно удален`, 'success', 'Удаление лида');
...
async function openLeadDrawer(leadId, state) {
    ...
    modalWrapper.querySelector('#delete-lead-drawer-btn').onclick = async () => {
        if (await confirm('Вы уверены, что хотите безвозвратно удалить это обращение?')) {
            try {
                ...
                close();
                await state.fetchLeads();
                window.showToast('Обращение успешно удалено', 'success', 'Удаление обращения');
            } catch (err) {
                alert('Ошибка при удалении: ' + err.message);
            }
        }
