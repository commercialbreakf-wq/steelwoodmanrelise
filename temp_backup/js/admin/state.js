...
class AdminState {
    ...
    _showSessionExpired() {
        ...
        overlay.innerHTML = `
            <div style="background:#1d1b19;border:1px solid rgba(255,176,204,0.2);border-radius:24px;padding:40px 48px;text-align:center;max-width:400px;box-shadow:0 40px 80px rgba(0,0,0,0.6);">
                <span style="font-family:'Material Symbols Outlined';font-size:48px;color:#ffb0cc;display:block;margin-bottom:16px;">lock</span>
    ...
    async fetchProducts() {
        ...
        try {
            const data = await this.authenticatedFetch('/api/admin/products');
    ...
    async fetchUsers() {
        this.setLoading(true);
        try {
            const data = await this.authenticatedFetch('/api/admin/users');
    ...
    async fetchOrders() {
        this.setLoading(true);
        try {
            const data = await this.authenticatedFetch('/api/admin/orders');
    ...
    async fetchLeads() {
        this.setLoading(true);
        try {
            const data = await this.authenticatedFetch('/api/admin/leads');
