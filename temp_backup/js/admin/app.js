...
const initAdminApp = async () => {
    try {
        ...
        // Auto-refresh Supabase token to prevent session expiry
        try {
            const SUPABASE_URL = 'https://drbknuvnsyonmeudoleo.supabase.co';
            const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRyYmtudXZuc3lvbm1ldWRvbGVvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDcyNTMzMzEsImV4cCI6MjA2MjgyOTMzMX0.R78l1Mx3wDgUjz0kZ0OkX5Rz8E8k1AGMnJMqNqbr2ak';
            if (window.supabase) {
                const sb = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
                sb.auth.onAuthStateChange((event, session) => {
                    if (session && session.access_token) {
                        localStorage.setItem('metal_token', session.access_token);
                        console.log('[AUTH] Token auto-refreshed');
                    }
                });
                // Trigger initial session restore
                sb.auth.getSession().then(({ data }) => {
                    if (data?.session?.access_token) {
                        localStorage.setItem('metal_token', data.session.access_token);
                    }
                });
            }
        } catch (e) {
            console.warn('[AUTH] Supabase auto-refresh setup failed:', e.message);
        }
        ...
        const renderProductsView = async (container) => {
                ...
                if (type === 'l1') {
                    ...
                    document.querySelectorAll('.delete-l1-btn').forEach(btn => {
                        btn.onclick = async (e) => {
                            ...
                            if (await confirm(`Вы уверены, что хотите удалить категорию "${name}"? Вместе с ней будут удалены все подкатегории и товары.`)) {
                    ...
                    document.querySelectorAll('.delete-l2-btn').forEach(btn => {
                        btn.onclick = async (e) => {
                            e.stopPropagation();
                            const name = btn.dataset.name;
                            if (await confirm(`Вы уверены, что хотите удалить подкатегорию "${name}"? Вместе с ней будут удалены все вложенные товары.`)) {
            ...
            const updateFilterOptions = () => {
                const currentL1 = filterL1.value;
                const currentL2 = filterL2.value;

                const l1Set = new Set(currentProducts.map(p => p.parent_category).filter(Boolean));
                const l2Set = new Set(currentProducts.filter(p => !currentL1 || p.parent_category === currentL1).map(p => p.category).filter(Boolean));

                filterL1.innerHTML = '<option value="">Все категории (L1)</option>' + 
                    Array.from(l1Set).sort().map(l => `<option value="${l}" ${currentL1 === l ? 'selected' : ''}>${l}</option>`).join('');
                
                filterL2.innerHTML = '<option value="">Все подкатегории (L2)</option>' + 
                    Array.from(l2Set).sort().map(l => `<option value="${l}" ${currentL2 === l ? 'selected' : ''}>${l}</option>`).join('');
            };
            ...
            filterL1.addEventListener('change', () => {
                ...
                let currentL2 = filterL2.value;
                if (currentL1 && !l2Set.has(currentL2)) {
                    currentL2 = '';
                }
  ...
  <Table>
   ...
   <Row ss:Height="28">
    ...
    <Cell ss:StyleID="StringCell"><Data ss:Type="String">${escapeXml(p.weight)}</Data></Cell>
                    ...
                    onImportPrice: async (file) => {
                        ...
                        reader.onload = async (e) => {
                            try {
                                ...
                                if (updates.length > 0) {
                                    await state.bulkUpdateProducts(updates);
                                    window.showToast(`Обновлено ${updates.length} товаров`, 'success', 'Импорт прайс-листа');
                                } else {
                                    window.showToast('Нет данных для обновления', 'warning', 'Импорт прайс-листа');
                                }
                            } catch (err) {
            ...
            // Subscribe to updates (store unsubscribe fn to avoid memory leaks on view switch)
            const unsubProducts = state.on('products:updated', (products) => {
                currentProducts = products;
                applyFilters();
            });
            // Cleanup when view is changed
            container._cleanup = () => { if (unsubProducts) unsubProducts(); };
        };
